...
<template>
  <OADialog v-model="dialogDeleteVisible" @submit="onDeleteInform" title="提示">
    ...
    <el-card>
      <el-table :data="informs" style="width: 100%">
        <el-table-column label="标题">
          <template #default="scope">
            <el-badge v-if="scope.row.reads.length==0" is-dot class="item">
              <RouterLink :to="{ name: 'informdetail', params: { pk: scope.row.id } }">{{ scope.row.title }}</RouterLink>
            </el-badge>
            <RouterLink v-else :to="{ name: 'informdetail', params: { pk: scope.row.id } }">{{ scope.row.title }}</RouterLink>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-button @click='onShowDelete(scope.$index)' type="danger" v-if="scope.row.author.uid == authStore.user.uid" icon="Delete" />
          </template>
        </el-table-column>
      </el-table>
      <template #footer>
