...
let absentRules = reactive({
  title: [{required: true, message: '请输入标题', trigger: 'blur'}],
  absent_type_id: [{required: true, message: '请输入请假类型', trigger: 'change'}],
  date_range: [{required: true, message: '请输入请假日期', trigger: 'blur'}],
...
<template>
  <OAMain title="个人考勤">
    <el-card style="text-align: right">
      <el-button type="primary" @click="onShowAbsent">
  ...
  <OADialog title="发起请假" v-model="dialogFormVisible" @submit="onSubmitAbsent">
    <el-form :model="absentForm" :rules="absentRules" ref="absentFormRef">
      ...
      <el-form-item label="请假类型" :label-width="formLabelWidth" prop="absent_type_id">
        <el-select v-model="absentForm.absent_type_id" placeholder="请选择请假类型">
          <el-option v-for="item in absent_types" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="审批领导" :label-width="formLabelWidth">
        <el-input :value="responder_str" readonly disabled autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="请假理由" :label-width="formLabelWidth" prop="request_content">
        <el-input type="textarea" v-model="absentForm.request_content" autocomplete="off"></el-input>
      </el-form-item>
    </el-form>
  </OADialog>
